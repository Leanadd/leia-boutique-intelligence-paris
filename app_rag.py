...
def load_ml_models():
    try:
        ...
        le_collection = joblib.load("le_collection.pkl")
        le_occasion = joblib.load("le_occasion.pkl")
        return model, le_tier, le_collection, le_occasion
    except Exception as e:
        print(f"ML models not found : {e}")
        return None, None, None, None

ml_model, le_tier, le_collection, le_occasion = load_ml_models()

# Direct CSV lookup for deterministic client search
@st.cache_data
def load_clients_df():
    try:
        df = pd.read_csv("knowledge_base/client_profiles.csv", encoding='utf-8')
...
def load_purchases_df():
    try:
        df = pd.read_csv("knowledge_base/purchase_history.csv", encoding='utf-8')
        return df
    except Exception as e:
        return pd.DataFrame()

clients_df = load_clients_df()
purchases_df = load_purchases_df()
...
def load_ml_models():
    try:
        ...
        clients = pd.read_csv("knowledge_base/client_profiles.csv", encoding='utf-8')
        purchases = pd.read_csv("knowledge_base/purchase_history.csv", encoding='utf-8')
        products = pd.read_csv("knowledge_base/leia_products.csv")
        
        df = purchases.merge(
            clients[["client_id","age","total_spent_usd","purchase_count",
        ...
        df["vip_tier_enc"]              = le_tier.fit_transform(df["vip_tier"].fillna("Gold"))
        df["preferred_collections_enc"] = le_collection.fit_transform(df["preferred_collections"].fillna("Hatching"))
        df["occasion_enc"]              = le_occasion.fit_transform(df["occasion"].fillna("Self-gift"))
        df["budget_max"]                = df["budget_range"].str.split("-").str[1].astype(float)
        
        features = [
            "age", "total_spent_usd", "purchase_count",
            "vip_tier_enc", "preferred_collections_enc",
            "occasion_enc", "budget_max", "price_usd"
        ]
        ...
        print(f"✅ ML model trained — accuracy: {model.score(X_test, y_test):.2%}")
        
        return model, le_tier, le_collection, le_occasion, products
    
    except Exception as e:
        print(f"❌ ML model error : {e}")
        return None, None, None, None, None

ml_model, le_tier, le_collection, le_occasion, products_ml = load_ml_models()

def find_client_in_csv(user_input, clients_df, purchases_df):
    """
    Deterministic client lookup from CSV
    Extracts first name from question and searches directly
    Returns formatted client context string
    """
    if clients_df.empty:
        return None
    ...
    potential_names = [w.strip("?.,!") for w in words 
                      if w[0].isupper() 
                      and w not in ["I","The","A","An","My","How","What",
                                    "Who","When","Where","Tell","Give","Can",
                                    "Could","Would","Please","Tomorrow","Today"]]
    ...
    for name in potential_names:
        match = clients_df[
            clients_df['first_name'].str.lower() == name.lower()
        ]
        if not match.empty:
            client = match
            break
        # try last name
        match = clients_df[
            clients_df['last_name'].str.lower() == name.lower()
        ]
        if not match.empty:
            client = match
            break
    ...
    client_purchases = purchases_df[
        purchases_df['client_id'] == c['client_id']
    ][['date','product_name','collection','price_usd']].sort_values('date', ascending=False)
    
    purchases_str = ""
    for _, p in client_purchases.head(5).iterrows():
        purchases_str += f"\n  - {p['product_name']} ({p['collection']}) ${p['price_usd']:,} on {p['date']}"
    
    # format client context
    context = f"""
CLIENT PROFILE — DIRECT LOOKUP
================================
Name: {c['first_name']} {c['last_name']}
Pronouns: {c['preferred_pronouns']}
VIP Tier: {c['vip_tier']}
Total Spent: ${c['total_spent_usd']:,}
Purchase Count: {c['purchase_count']}
Last Visit: {c['last_visit_date']}
Birthday: {c['birthdate']}
Preferred Collections: {c['preferred_collections']}
Style: {c['style_profile']}
Budget Range: {c['budget_range'].replace('-', ' to ')}
Notable Purchases: {c['notable_purchases']}
Notes: {c['notes']}
Preferred Advisor: {c['preferred_advisor']}

RECENT PURCHASE HISTORY (last 5):
{purchases_str if purchases_str else "No purchases found"}
================================
"""
    # inject product catalog to prevent hallucination
    try:
        products_ref = pd.read_csv("knowledge_base/leia_products.csv")
        
        # filter by client's preferred collections
        preferred = [col.strip() for col in str(c['preferred_collections']).split(',')]
        filtered_products = products_ref[
            products_ref['collection'].isin(preferred)
        ][['name', 'collection', 'price_usd']]
        
        products_str_catalog = "\nAVAILABLE PRODUCTS IN CLIENT'S PREFERRED COLLECTIONS:\n"
        for _, p in filtered_products.iterrows():
            products_str_catalog += f"  - {p['name']} ({p['collection']}) ${p['price_usd']:,}\n"
        
        products_str_catalog += "\nCRITICAL : ONLY recommend products from this list.\nNEVER invent product names or prices.\n"
        
        context += products_str_catalog

    except Exception as e:
        pass

    # inject ML recommendations if model is available
    try:
        ml_recs = get_ml_recommendations(
            c['client_id'], clients_df, purchases_df,
            ml_model, le_tier, le_collection, le_occasion, products_ml
        )
        if ml_recs:
            context += ml_recs
            context += "\nPRIORITY : Use the ML recommendations above as your primary suggestion list.\nOnly suggest products from this list or from the AVAILABLE PRODUCTS section.\n"
    except Exception as e:
        pass
...
def find_clients_by_tier(user_input, clients_df):
    """
    Direct CSV lookup for VIP tier queries
    Returns formatted overview of clients in a specific tier
    """
    if clients_df.empty:
        return None
    
    # detect which tier is being asked about
    tier_map = {
        "diamond": "Diamond",
        "platinum": "Platinum", 
        "gold": "Gold",
    ...
    tier_clients = clients_df[clients_df['vip_tier'] == detected_tier].copy()
    
    if tier_clients.empty:
        return f"No {detected_tier} tier clients found in the knowledge base."
    
    context = f"""
DIRECT LOOKUP — {detected_tier.upper()} TIER CLIENTS
================================
Total : {len(tier_clients)} clients
Avg spend : ${tier_clients['total_spent_usd'].mean():,.0f}
Avg purchases : {tier_clients['purchase_count'].mean():.1f}

CLIENT LIST :
"""
    for _, c in tier_clients.sort_values('total_spent_usd', ascending=False).iterrows():
        context += f"\n- {c['first_name']} {c['last_name']} | ${c['total_spent_usd']:,} | {c['purchase_count']} purchases | Last visit: {c['last_visit_date']} | {c['preferred_collections']}"
    
    context += f"""

COLLECTIONS BREAKDOWN :
{tier_clients['preferred_collections'].str.split(', ').explode().value_counts().to_string()}

LÉIA collections available : Eclipse, Hatching, Amazon, Vanta
ONLY reference these 4 collections in your response.
================================
"""
    return context


def get_ml_recommendations(client_id, clients_df, purchases_df, 
                            ml_model, le_tier, le_collection, 
                            le_occasion, products_ml, top_n=3):
    """
    ML-powered recommendation for a specific client
    Uses Random Forest trained on purchase history
    """
    if ml_model is None:
        return None
    ...
    already_bought = purchases_df[
        purchases_df['client_id'] == client_id
    ]['product_id'].tolist()
    
    # budget max
    budget_max = int(str(client['budget_range']).split('-')[1])
    
    # candidate products — not owned, within budget
    candidates = products_ml[
        (~products_ml['product_id'].isin(already_bought)) &
        (products_ml['price_usd'] <= budget_max * 1.2)
    ].copy()
    
    if candidates.empty:
        return None
    
    # encode client features
    try:
        tier_enc = le_tier.transform([client['vip_tier']])[0]
    except:
        tier_enc = 0
    try:
        col_enc = le_collection.transform([client['preferred_collections']])[0]
    except:
        col_enc = 0
    ...
    candidates["vip_tier_enc"]              = tier_enc
    candidates["preferred_collections_enc"] = col_enc
    candidates["occasion_enc"]              = occasion_enc
    ...
    probas = ml_model.predict_proba(X_candidates)
    ...
    # format output
    result = "\nML-POWERED RECOMMENDATIONS (Random Forest):\n"
    for rank, (_, row) in enumerate(top3.iterrows(), 1):
        result += f"  #{rank} {row['name']} ({row['collection']}) ${row['price_usd']:,} — confidence: {row['score']:.0%}\n"
...
- One space before and after every dash —
- Each recommended piece on its own line
- Never compress text — always leave spaces between words and symbols
...
→ Client Name : [First name Last name] | Tier :[VIP Tier] | Style : [Style] | Budget range :[Budget range]
...
When ML confidence scores are present in the context, translate them 
into natural advisor language instead of showing percentages :
- High confidence (>50%) : "a strong match for her profile"
- Medium confidence (25-50%) : "worth exploring"
- Low confidence (<25%) : "a more exploratory suggestion, 
  slightly outside her typical pattern — could be a refreshing option"
Never show the raw percentage number.
...
→ Example: "Lucie arrives tomorrow, how do I prepare?" 
   → Example: "Lucie arrives tomorrow, how do I prepare?"
   → Client Name:[Client Name] | VIP Tier:[VIP Tier] | Style:[Style] | Budget range:[Budget]
     → Already owns : [piece 1 ($price)], [piece 2 ($price)]
     → Birthday : [date — flag if within 30 days]
     → Recommended pieces :
     1- [Piece name] ($price) — [why it fits her style]
     2- [Piece name] ($price) — [why it fits, note if slightly above budget]
     3- [Piece name] ($price) — [why it's the natural next step]
...
- Simulate predictive scores (churn, LTV) — 
  ...
  FORMAT RULES :
- Always use $ for prices, never backticks
- Always name the specific clients, never just numbers
- Include : name, VIP tier, total spent, last visit, preferred collections

RESPONSE STRUCTURE for segment queries :
→ Number of clients in segment
→ List each client : Name : [Client Name] | Total Spent : [Total Spent] | Last Visit : [Last Visit] | Preferred Collections : [Preferred Collections]
→ Key pattern observed
→ Recommended next action for the advisor

CRITICAL — ZERO HALLUCINATION :
ONLY use client names, collections, and data explicitly 
found in the context provided.
LÉIA has exactly 4 collections : Eclipse, Hatching, Amazon, Vanta.
Any other collection name is an invention — never use it.
If you cannot find Diamond clients in the context, say :
"I couldn't retrieve Diamond client profiles from the knowledge base.
Please query a specific client name for accurate data."
...
if user_input:
    ...
    with st.spinner("🔍 Searching knowledge base..."):
        ...
        clienteling_keywords = [
            "arriving", "coming", "visit", "prepare", "tomorrow",
            "appointment", "welcoming", "meeting"
        ]
        
        is_clienteling = any(kw in user_input.lower() for kw in clienteling_keywords)
        
        if is_clienteling:
            client_context = find_client_in_csv(user_input, clients_df, purchases_df)
            rag_context = get_relevant_context(
                kb,
                user_input + " products collection pieces recommendation",
                k=15
            )
            if client_context:
            ...
            crm_keywords = ["diamond", "platinum", "gold", "member", "tier", "segment", "vip"]
            
            if any(kw in user_input.lower() for kw in crm_keywords):
                tier_context = find_clients_by_tier(user_input, clients_df)
                rag_context = get_relevant_context(kb, user_input, k=10)
                
                if tier_context:
                    context = tier_context + "\n\n" + rag_context
                else:
                    context = get_relevant_context(kb, user_input, k=25)
            else:
                context = get_relevant_context(kb, user_input, k=25)
